...
pygame.init()
white=(255,255,255) #colores
black=(0,0,0) #colores
red=(255,0,0)
green=(0,255,0)
blue=(0,0,255)
dw=600 #ancho de la ventana y alto
dh=476
screen=pygame.display.set_mode([dw,dh]) #guardamos las ventana en una variable
pygame.display.set_caption('Flappy Bird')
pimg=[pygame.image.load(str(i)+'.png') for i in range(1,5)] #cargar 4 imagenes
clock=pygame.time.Clock() 
vec=pygame.math.Vector2 #vectores
...
blist=[[50,310],[60,300],[70,290],[80,280],[90,270],[100,260],[110,250],[120,240],[130,230],[140,220],[150,210],[160,200],[170,190],[180,180],
      ...
      ,[290,70],[300,60],[310,50]] #determinar pocisiones de los tupos
...
def start():
   t1 = threading.Thread(target=when_start) #sonidos
   t1.start()

def out():
   t2 = threading.Thread(target=when_quit)
   t2.start()

def crash():
   t3 = threading.Thread(target=when_crash)
   t3.start()

class Bird(pygame.sprite.Sprite):
   def __init__(self,game):
      super().__init__()
      self.image=pimg[0] #imagine 1
      self.image=pygame.transform.scale(self.image,(100,85)) #medidas
      self.rect=self.image.get_rect()
      self.vel=vec(0,0)
      self.rect.center=(dw/2,dh/2) #centra el paj
      self.acc=vec(0,0)
      self.pos=vec(self.rect.center) #refresca el pajaro
      self.fc=0
   def update(self):
      self.acc=vec(0,1.5)
      self.vel=vec(0,0)
      keys=pygame.key.get_pressed()
      if keys[pygame.K_SPACE]: #mover el pajaro
         self.acc.y=-1.5
         if self.fc+1<28: #para hacer la animacion
            self.fc+=1
            self.image=pimg[self.fc//7]
            self.image=pygame.transform.scale(self.image,(100,85))
         else:
            self.fc=0 #se reinicia a estado normal
      else:  #pajaro quieto
         self.image=pimg[0] #arreglo de fotos
         self.image=pygame.transform.scale(self.image,(100,85))
      self.vel+=self.acc
      self.pos+=self.vel+0.5*self.acc #aumenta la velocidad
...
class TBlock(pygame.sprite.Sprite): #tubos de arriba
...
class BBlock(pygame.sprite.Sprite): #tupos abajo
...
class Game: #m
   def __init__(self):
      start() #trae hilos para la reproduccion de sonidos en 2do plano
      self.bgx=0
      self.x=650
      self.h1=180
      self.h2=180
   ...
   def over(self): 
      ...
      while wait:
         for event in pygame.event.get():
            if event.type==pygame.QUIT: 
               out()#tercel hilo audio cuando cierre el juego
               pygame.quit()
            if event.type==pygame.KEYDOWN:
               if event.key==pygame.K_RETURN:
                  wait=0
   ...
   def draw(self): #dibuja elementos en pantallla
   ...
   def event(self):
      for event in pygame.event.get():
         ...
         if event.type==pygame.KEYDOWN:
               if event.key==pygame.K_RETURN:
                  self.pause()
   def run(self): #para que siga funcionando
      while 1:
         self.event()
         self.update()
         self.draw()
